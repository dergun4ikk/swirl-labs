...
| Use class(cls_vect) to confirm that sapply() simplified the result to
| a character vector.
...
| 194) containing each color. Since each column is just a bunch of 1s
| and 0s, the arithmetic mean of each column will give us the proportion
| of 1s. (If it's not clear why, think of a simpler situation where you
...
| range function to each column of flag_shapes. Don't worry about
...
| by lapply(). It has been successful in doing so for each of the
| examples we've looked at so far. Let's look at an example where
| sapply() can't figure out how to simplify the result and thus returns
| a list, no different from lapply().
...
| determine the length of each element of unique_vals (i.e. the number
| of unique values for each variable). Simplify the result, if possible.
...
| Apply the length() function to each element of the unique_vals list
| using sapply(). Remember, no parentheses after the name of the
| function you are applying (i.e. length).
...
| defined, thus requiring you to write your own. Writing functions in R
| is beyond the scope of this lesson, but let's look at a quick example
...
| of the unique_vals list that you just created. Since each element of
| the unique_vals list is a vector and we're not aware of any built-in
...
| 'dummy variable' that takes on the value of each element of
| unique_vals, in turn.
